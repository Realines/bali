...
class DataConsultation(models.Model):
    ...
    date_submission = models.DateTimeField(verbose_name='Дата подачи заявки', auto_now_add=True)
...
class Project(models.Model):
    ...
    view = models.TextField(verbose_name='Вид', help_text='Открывающийся из виллы вид')
    location = models.ForeignKey('Location', verbose_name='Локация', on_delete=models.SET_NULL,
                                 related_name='projects', related_query_name='project', null=True)
    date_added = models.DateField(verbose_name='Дата добавления', auto_now_add=True)
    priority = models.SmallIntegerField(verbose_name='Приоритет проекта',
                                        help_text='От этого значения будет зависит выборка рекомендаций')
    categories = models.ManyToManyField('Category', verbose_name='Категории',
                                        related_name='projects', related_query_name='project')
    invested = models.BooleanField(verbose_name='Проект подлежит инвестированию', default=False)
...
class LocationFact(models.Model):
    ...
    location = models.ForeignKey(Location, verbose_name='Локация', on_delete=models.CASCADE,
                                 related_name='facts', related_query_name='fact', null=True)
...
class Quiz(models.Model):
    ...
    order_type = models.CharField(max_length=4, verbose_name='Тип заказа', choices=OrderType.choices)
    distance_to_sea = models.IntegerField(verbose_name='Расстояние до моря',
                                          null=True, blank=True)
    profit_assessment = models.CharField(max_length=4, verbose_name='Важность доходности',
                                         choices=Assessments.choices, null=True, blank=True)
    assessment_district = models.CharField(max_length=4, verbose_name='Важность района',
                                           choices=Assessments.choices, null=True, blank=True)
